...
```markdown
[디자인 목표] 매물 상세와 건물 안전 정보를 한눈에 확인하는 카드형 상세 화면 디자인.

[시각적 디자인 스펙]
1. 매물 상세 Panel: 매물 유형, 썸네일, 주소, 보증금, 월세, 관리비, 면적 등 핵심 정보와 관심 하트 아이콘 배치
2. 건물 안전 정보 Panel: 건물 인증 여부, 준공 연도, 거주 이력 등 신뢰 정보를 2분할 요약 카드로 구성
3. 안전 점수 Panel: 점수와 상태에 따라 색상이 변하는 원형 차트, 안전 상태 뱃지 및 세부 지표 표시
4. Detail Link: `[안전 점수 상세보기]` 터치 시 산정 기준 모달 오픈
```
...
[시각적 디자인 스펙]
1. 금융상품 Panel: 대출·보증 등 금융상품 정보를 접기/펼치기 가능한 Accordion 카드로 구성
2. 편의시설 Panel: 교통, 편의점, 병원 등 주변 시설과 거리 정보를 접기/펼치기 가능한 Accordion 카드로 구성
3. 실거주 커뮤니티: 사용자 의견 미리보기 카드와 `[댓글 전체보기]` 링크 배치
```
...
[시각적 디자인 스펙]
1. 안전 점수 상세보기 Modal: Dimmed Overlay, 제목, 닫기(X) 버튼과 점수 산정 기준 리스트 구성
2. 점수 기준: 치안, CCTV, 가로등, 대중교통, 건물 안전성 등 항목별 아이콘과 설명 표시
3. 댓글 전체보기 Modal: 실거주 커뮤니티의 전체 댓글 목록과 댓글 입력창, 등록 버튼 배치
```
