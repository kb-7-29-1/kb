...
<template>
  <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl w-full max-w-md p-7 relative">
      ...
      <div class="find-tab-control flex rounded-full bg-gray-100 p-1 mb-6">
        <span
          class="find-tab-indicator"
          :class="{ 'is-password': tab === 'findPassword' }"
          aria-hidden="true"
        ></span>
        <button
          @click="switchTab('findId')"
          :class="tab === 'findId' ? 'font-semibold text-indigo-600' : 'text-gray-500'"
          class="find-tab-button flex-1 rounded-full py-2 text-sm"
        >
          아이디 찾기
        </button>
        <button
          @click="switchTab('findPassword')"
          :class="tab === 'findPassword' ? 'font-semibold text-indigo-600' : 'text-gray-500'"
          class="find-tab-button flex-1 rounded-full py-2 text-sm"
        >
          비밀번호 찾기 & 즉시 변경
        </button>
      ...
      <div :key="tab" class="find-tab-content">
        <!-- 아이디 찾기 -->
        <div v-if="tab === 'findId'">
...
.find-tab-indicator.is-password {
...
.find-tab-content {
  animation: find-tab-content-enter 0.22s ease-out;
}

@keyframes find-tab-content-enter {
  from {
    opacity: 0;
