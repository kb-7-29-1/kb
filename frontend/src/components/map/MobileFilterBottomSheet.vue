...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <!-- 1. 바텀시트 상단 드래그 핸들 & 닫기 버튼 -->
        <div class="px-4 pt-3 pb-1 flex items-center justify-between bg-white relative">
          <span class="w-8 h-1 bg-slate-200 rounded-full mx-auto absolute top-2 left-1/2 -translate-x-1/2"></span>
          <div class="text-sm font-black text-slate-900 mt-2">필터 상세 설정</div>
          <button
            type="button"
            class="w-7 h-7 rounded-full bg-slate-100 text-slate-500 font-bold flex items-center justify-center text-xs hover:bg-slate-200 transition-colors mt-1"
            @click="close"
          >
            ✕
          </button>
        ...
        <div class="flex border-b border-slate-200 bg-white sticky top-0 z-20 px-4">
          <button
            ...
            :class="activeTab === 'general' ? 'text-blue-600 font-extrabold' : 'text-slate-500 hover:text-slate-800'"
            @click="activeTab = 'general'"
          >
            <span>🎨</span>
            <span>전체 조건 필터</span>
            <div v-if="activeTab === 'general'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-full"></div>
          </button>

          <button
            type="button"
            class="flex-1 py-3 text-xs font-bold transition-all relative flex items-center justify-center gap-1.5"
            :class="activeTab === 'amenity' ? 'text-blue-600 font-extrabold' : 'text-slate-500 hover:text-slate-800'"
          ...
          >
            ...
            <div v-if="activeTab === 'amenity'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600 rounded-full"></div>
          </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-5">
          ...
          <div v-else class="space-y-4">
            <div class="p-3 bg-blue-50/80 rounded-2xl border border-blue-200/80 text-xs text-blue-900 font-bold flex items-center gap-2">
              <span>🛍️</span>
