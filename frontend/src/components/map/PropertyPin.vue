...
<template>
  ...
  >
    <div
      class="flex w-max items-center gap-2 whitespace-nowrap rounded-full border px-3.5 py-2 text-sm font-bold shadow-lg transition-all"
      :class="[
    ...
    >
      ...
      <span
        class="shrink-0 rounded-md px-2 py-0.5 text-[11px] font-bold"
        :class="isSelected ? 'bg-white/20 text-white' : safetyPinTheme.badge"
      >
        {{ safetyScore }}점
      </span>
    </div>
    <div class="-mt-2 h-3 w-3 rotate-45" :class="safetyPinTheme.pointer"></div>
