<template>
  <div class="modal-backdrop" @click.self="emit('close')">
    <div class="modal-container">
      ...
      <div
        v-if="isCalculating"
        class="calculation-loading calculation-loading--full"
        role="status"
        aria-live="polite"
      >
      ...
      <template v-else>
        ...
        <section
          class="score-section"
          :style="{ backgroundColor: safetyScoreTheme.background }"
        >
          <div
            v-if="isCalculating"
            class="calculation-loading"
            role="status"
            aria-live="polite"
          >
            <i
              class="fa-solid fa-spinner calculation-loading__icon"
              aria-hidden="true"
            ></i>
            <div>
              <strong>안전점수를 계산하고 있어요</strong>
          ...
          <div class="score-headline">
            <span
              class="score-number"
              :style="{ color: safetyScoreTheme.color }"
              >{{ safetyScore === null ? '-' : `${safetyScore}점` }}</span
            >
            <span
              class="score-status"
              :style="{ color: safetyScoreTheme.color }"
          ...
          <ul class="factor-list">
            ...
            <li class="factor-item">
              ...
              <div class="info-content">
                ...
                <template
                  v-if="
                    nearestPoliceDistanceMeters !== null &&
                    nearestPoliceDistanceMeters <= 500
                  "
                >
                  <div class="info-desc">
                    가장 가까운 파출소 약 {{ nearestPoliceDistanceMeters }}m
                  </div>
                  <div class="info-desc">
                    도보 약 {{ nearestPoliceWalkMinutes }}분
                  </div>
                </template>
        ...
        <section class="notice-box">
          <p class="notice-main">
            점수는 귀갓길 거리와 경로 유형, CCTV·가로등의 분포, 파출소 접근성을
            함께 반영해 계산해요.
          </p>
          <div class="notice-sub">
            ...
            <span
              >공공데이터를 바탕으로 분석하며, 실제 환경이나 체감 안전과 차이가
              있을 수 있어요.</span
            >
          </div>
        </section>
...
const safetyGrade = computed(() => {
  if (safetyScore.value === null) return '데이터 미제공';
...
const safetyScoreTheme = computed(() => {
  ...
  if (safetyScore.value >= 80)
    return { color: '#22a06b', background: '#ecfdf3' };
  if (safetyScore.value >= 60)
    return { color: '#e69a1d', background: '#fff7e8' };
...
const streetLampAverageGapMeters = computed(() => {
  ...
  if (calculatedGap !== null && calculatedGap !== undefined)
...
const formatAverageGap = (label, value) => {
  if (value !== null)
    return `${label} 간 평균 거리 약 ${Math.round(Number(value))}m`;
  return props.isCalculating
    ? `${label} 간 평균 거리 계산 중`
    : `${label} 간 평균 거리 계산 결과 없음`;
};
...
const nearestPoliceWalkMinutes = computed(() => {
  if (
    nearestPoliceDistanceMeters.value === null ||
    nearestPoliceDistanceMeters.value > 500
  )
    return null;
...
const hasPoliceStation = computed(() =>
  Boolean(props.property?.hasPoliceStation),
);
</script>
