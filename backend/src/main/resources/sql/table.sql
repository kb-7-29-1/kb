...
CREATE TABLE destinations (
    ...
    dest_name VARCHAR(100) NOT NULL UNIQUE,
    dest_address VARCHAR(255) NULL,
    CONSTRAINT chk_dest_latitude CHECK (
        dest_latitude BETWEEN -90 AND 90
    ),
    CONSTRAINT chk_dest_longitude CHECK (
        dest_longitude BETWEEN -180 AND 180
    ),
...
CREATE TABLE properties (
    ...
    CONSTRAINT chk_property_latitude CHECK (
        property_latitude BETWEEN -90 AND 90
    ),
    CONSTRAINT chk_property_longitude CHECK (
        property_longitude BETWEEN -180 AND 180
    ),
    CONSTRAINT chk_properties_del_yn CHECK (del_yn IN ('N', 'Y')),
    UNIQUE KEY uq_property_location (
        property_latitude,
        property_longitude
    ),
...
CREATE TABLE onboardings (
    ...
    CONSTRAINT chk_onboarding_safety_score CHECK (
        min_safety_score BETWEEN 0 AND 100
    )
);
...
CREATE TABLE property_safety (
    ...
    CONSTRAINT chk_property_safety_score CHECK (
...
CREATE TABLE property_safety_route (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (property_id, destination_id),
    CONSTRAINT fk_safety_route_property_safety FOREIGN KEY (property_id, destination_id) REFERENCES property_safety (property_id, destination_id) ON DELETE CASCADE,
...
CREATE TABLE property_amenities (
    ...
    CONSTRAINT chk_amenity_latitude CHECK (
        amenity_latitude BETWEEN -90 AND 90
    ),
    CONSTRAINT chk_amenity_longitude CHECK (
        amenity_longitude BETWEEN -180 AND 180
    )
);
